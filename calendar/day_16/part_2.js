...
const onClose = () => {
  ...
  const tileEnergyLevels = startingPositions.map(([position, direction]) =>
    getTileEnergyLevels(position, direction)
  );

  const energizedTileCount = tileEnergyLevels.map(
    (tileEnergyLevels) => tileEnergyLevels.flat().filter(Boolean).length
  );

  console.log(Math.max(...energizedTileCount));
};
...
const getStartingPositions = () => {
  ...
  for (let i = 0; i < contraption.length; i++) {
    startingPositions.push([{ row: i, column: 0 }, directions.right]);
    startingPositions.push([
      { row: i, column: contraption[i].length - 1 },
      directions.left,
    ]);
  }

  for (let i = 0; i < contraption[0].length; i++) {
    startingPositions.push([{ row: 0, column: i }, directions.down]);
    startingPositions.push([
      { row: contraption.length - 1, column: i },
      directions.up,
    ]);
  }
